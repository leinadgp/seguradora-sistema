...
import { Plus, Search, CheckSquare, Phone, MessageSquare, Mail, RefreshCw, DollarSign, Folder, Users, Calendar, Trash2 } from 'lucide-react'
...
export default function Tarefas() {
  ...
  const { data: tarefas, create, update, remove } = useResource('tarefas')
      ...
      <div className={`bg-cyber-card rounded-2xl p-4 shadow-card border transition-all duration-200 hover:shadow-card-md hover:-translate-y-0.5 ${t.status === 'atrasada' ? 'border-red-200' : 'border-cyber-border/40'}`}>
        <div className="flex items-start gap-3">
          ...
          <div className="flex items-center gap-1 shrink-0">
            ...
            <button onClick={() => setConfirmDelete(t)} className="p-1.5 text-cyber-muted hover:text-cyber-red hover:bg-red-50 rounded-lg ml-0.5 transition-colors">
              <Trash2 size={12} />
            </button>
          </div>
        </div>
  ...
  return (
    <div className="space-y-4">
      ...
      {confirmDelete && (
        ...
        >
          <p className="text-sm text-cyber-text">Excluir a tarefa <strong className="text-cyber-red">"{confirmDelete.titulo}"</strong>?</p>
