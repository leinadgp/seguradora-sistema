...
const comissaoPorCorretor = [
  { nome: 'Pedro Lima', valor: 36442 },
  { nome: 'Carlos Silva', valor: 25370 },
  { nome: 'Roberto Alves', valor: 16552 },
  { nome: 'Ana Santos', valor: 14500 },
  { nome: 'Fernanda Costa', valor: 0 },
]

const taxaConversao = [
  { mes: 'Jan', propostas: 8, convertidas: 5 },
  { mes: 'Fev', propostas: 11, convertidas: 7 },
  { mes: 'Mar', propostas: 9, convertidas: 6 },
  { mes: 'Abr', propostas: 13, convertidas: 8 },
  { mes: 'Mai', propostas: 15, convertidas: 10 },
  { mes: 'Jun', propostas: 12, convertidas: 8 },
]

const leadsPorOrigem = [
  { name: 'Site', value: 3 },
  { name: 'Indicação', value: 3 },
  { name: 'Redes Sociais', value: 1 },
  { name: 'Prospecção', value: 2 },
...
export default function Relatorios() {
  ...
  const apolicesPorTipo = Object.entries(
    apolices.reduce((acc, a) => { acc[a.tipoSeguro] = (acc[a.tipoSeguro] || 0) + 1; return acc }, {})
  ).map(([name, value]) => ({ name, value }))

  const totalComissao = comissoes.reduce((a, c) => a + c.valor, 0)
  const totalPremios = apolices.reduce((a, ap) => a + (ap.premioBruto || 0), 0)
  const txConversao = Math.round((10 / 15) * 100)
        ...
        <ChartCard title="Produção Mensal" subtitle="Prêmios emitidos em R$ (últimos 6 meses)">
          <ResponsiveContainer width="100%" height={200}>
            <BarChart data={producaoMensal} barSize={30}>
              ...
              <Tooltip formatter={v => fmtMoeda(v)} contentStyle={{ background: '#0f172a', border: '1px solid rgba(6,182,212,0.2)', borderRadius: '12px', color: '#f8fafc' }} />
          ...
          <div className="flex items-center gap-4">
            <ResponsiveContainer width={160} height={160}>
              <PieChart>
                ...
                <Tooltip contentStyle={{ background: '#0f172a', border: '1px solid rgba(6,182,212,0.2)', borderRadius: '12px', color: '#f8fafc' }} />
              </PieChart>
            </ResponsiveContainer>
